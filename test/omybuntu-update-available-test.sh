...
git -C "$SOURCE_REPO" commit --quiet -m "legacy dev release"
GIT_COMMITTER_DATE="2025-01-01T00:00:00Z" \
  git -C "$SOURCE_REPO" tag -a v1.1.7_dev -m "legacy dev tag"

printf 'v1 dev\n' > "$SOURCE_REPO/state"
git -C "$SOURCE_REPO" commit --quiet -am "start v1 release line"
GIT_COMMITTER_DATE="2026-01-01T00:00:00Z" \
  git -C "$SOURCE_REPO" tag -a v1.0.0_dev1 -m "v1 dev tag"
...
GIT_COMMITTER_DATE="2027-01-01T00:00:00Z" \
  git -C "$SOURCE_REPO" tag -a v9.0.0_dev1 -m "unrelated dev tag"
...
[[ $output == *"up to date (v1.0.0_dev1)"* ]] || \
  fail "new v1 release ignores provisional tags outside the strict convention"
